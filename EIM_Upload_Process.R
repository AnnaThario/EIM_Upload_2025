...
setwd("C:\\Users\\athario\\OneDrive - King County\\Documents\\EIM_Upload_2025")
...
PSSB <- read.csv("PSSBDownload_2015-2025.csv") 
  #Just checking the column headers
  head(PSSB)

##Just the location information already in EIM
EIM_2014 <- read.csv("EIM_ExistingLocations_Download/EIMLocationDetails_2025Aug12_149.csv") 
  #Just checking the column headers
  head(EIM_2014)
...
location_template <- read.csv("EIMLocationTemplate.csv")
  #Just checking the column headers
  ...
  location_template[1:length(added_locations),] <- NA
  ...
  PSSB_loc2 <- read.csv("PSSB_LocationInfo.csv")
  SiteMaster <- read.csv("SiteMaster_NewSites.csv")
